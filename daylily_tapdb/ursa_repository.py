...
URSA_TEMPLATE_DEFINITIONS: tuple[TemplateDefinition, ...] = (
    TemplateDefinition(
    ...
    TemplateDefinition(
        "workflow/workset/gui-ready/1.0/",
        "workflow_template",
        "workflow_instance",
        "WS",
        "Ursa Workset",
    ...
    TemplateDefinition(
        "action/artifact/dewey-import/1.0/",
        "action_template",
        "action_instance",
        "DW",
        "Dewey Import Event",
    ...
    TemplateDefinition(
        ...
        "data_instance",
        "TV",
        "Ursa User Token Revision",
    ),
    TemplateDefinition(
        "integration/auth/user-token-usage/1.0/",
        "action_template",
        "action_instance",
        "TG",
        "Ursa User Token Usage",
    ),
    TemplateDefinition(
        "integration/auth/client-registration/1.0/",
        "data_template",
        "data_instance",
        "CG",
        "Ursa Client Registration",
    ),
    TemplateDefinition(
        "workflow/cluster/ephemeral-job/1.0/",
        "workflow_template",
        "workflow_instance",
        "CJ",
        "Ursa Ephemeral Cluster Job",
    ),
    TemplateDefinition(
        "action/cluster/ephemeral-job-revision/1.0/",
        "action_template",
        "action_instance",
        "CR",
        "Ursa Ephemeral Cluster Job Revision",
    ),
    TemplateDefinition(
        "action/cluster/ephemeral-job-event/1.0/",
        "action_template",
        "action_instance",
        "CE",
        "Ursa Ephemeral Cluster Job Event",
    ),
)


class UrsaTapdbRepository:
    """Ursa-specific TapDB helper surface.

    This module centralizes Ursa's direct TapDB query logic so application repos
    can use a thin adapter without importing SQLAlchemy or embedding raw SQL.
    """
    ...
    def list_instances_by_property(
        ...
        *,
        ...
        return cast(
            list[generic_instance],
            query.filter(generic_instance.json_addl[key].as_string() == value)
