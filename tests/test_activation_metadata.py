...
def test_pyproject_pins_published_cli_core_yo() -> None:
    ...
    assert "daylily-auth-cognito==2.1.5" in gui_dependencies
    assert all("daylily-cognito" not in dependency for dependency in gui_dependencies)
    assert "admin" not in data["project"]["optional-dependencies"]
...
def test_activate_uses_published_cli_core_yo_metadata_check() -> None:
    ...
    assert 'python -m pip install -e ".[cli,gui,aurora,dev]"' in text
